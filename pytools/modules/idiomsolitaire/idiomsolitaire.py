...
class IdiomSolitaire(QWidget):
    ...
    def __init__(self, parent=None, title='成语接龙小软件 —— Car的皮皮', **kwargs):
        super(IdiomSolitaire, self).__init__(parent)
        rootdir = os.path.split(os.path.abspath(__file__))[0]
        # 读取数据
        self.idiom_data, self.valid_idioms = self.readData(
            os.path.join(rootdir, 'resources/data.txt')
        )
        self.ai_answer = None
        # 定义界面
    ...
    def airound(self):
        ...
        idiom = idiom.strip()
        if (not self.isvalid(idiom)) or (
            self.ai_answer and idiom[0] != self.ai_answer[0][-1]
        ):
            QMessageBox.warning(
                self, '成语输入错误', '你输入的成语不对哦, 不可以耍小聪明哒!', QMessageBox.Yes | QMessageBox.No
            )
        else:
            self.user_explain_edit.setText(
                '读音: %s; 含义: %s'
                % (self.valid_idioms[idiom][0], self.valid_idioms[idiom][1])
            )
            if idiom[-1] in self.idiom_data:
                ...
                self.ai_explain_edit.setText(
                    '读音: %s; 含义: %s'
                    % (self.valid_idioms[answer[0]][0], self.valid_idioms[answer[0]][1])
                )
            else:
                QMessageBox.information(
                    self, '你赢啦', '电脑都接不上你的成语, 你太厉害啦!', QMessageBox.Yes | QMessageBox.No
                )
