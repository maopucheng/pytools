...
class VideoPlayer(QWidget):
    ...
    def __init__(self, parent=None, title='视频播放器 —— Car的皮皮', **kwargs):
        ...
        h_layout.addWidget(
            self.play_progress_label, 0, Qt.AlignCenter | Qt.AlignVCenter
        )
        h_layout.addWidget(
            self.play_progress_slider, 15, Qt.AlignVCenter | Qt.AlignVCenter
        )
        ...
        self.play_progress_slider.sliderReleased.connect(
            self.playProgressSliderReleased
        )
    ...
    def mute(self):
        if self.player.isMuted():
            self.mute_btn.setIcon(
                QIcon(os.path.join(self.rootdir, 'resources/sound.png'))
            )
            ...
            self.mute_btn.setIcon(
                QIcon(os.path.join(self.rootdir, 'resources/mute.png'))
            )
