...
class Fireworks(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.position = (
            Config.SCREENSIZE[0] * random.random(),
            Config.SCREENSIZE[1] - 10,
        )
        color = Utils().randc()
            ...
            if Config().USE_IMAGEPATH:
                screen.blit(
                    pygame.transform.scale(
                        pygame.image.load(Config.IMAGEPATH), Config.SCREENSIZE
                    ),
                    (0, 0),
                )
            if random.random() < 8 / 60:
                fireworks_list.add(Fireworks())
