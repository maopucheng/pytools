...
class Config:
    ...
    for name in ['pp', 'blackcat', 'whitecat', 'fox']:
    ...
    BAIDU_KEYS = random.choice(
        [
            [
                '25419425',
                'fct6UMiQMLsp53MqXzp7AbKQ',
                'p3wU9nPnfR7iBz2kM25sikN2ms0y84T3',
            ],
            [
                '24941009',
                '2c5AnnNaQKOIcTrLDTuY41vv',
                'HOYo7BunbFtt88Z0ALFZcFSQ4ZVyIgiZ',
            ],
            [
                '11403041',
                'swB03t9EbokK03htGsg0PKYe',
                'XX20l47se2tSGmet8NihkHQLIjTIHUyy',
            ],
        ]
    )
...
class DesktopPet(QWidget):
    ...
    def __init__(self, pet_type='pp', parent=None, **kwargs):
        ...
        # 显示
        self.resize(
            self.pet_images[0][0].size().width(), self.pet_images[0][0].size().height()
        )
        self.randomPosition()
        self.show()
        # 宠物动画动作执行所需的一些变量
    ...
    def randomLoadPetImages(self):
        ...
        for action in actions:
            pet_images.append(
                [
                    self.loadImage(
                        os.path.join(cfg.ROOT_DIR, pet_name, 'shime' + item + '.png')
                    )
                    for item in action
                ]
            )
