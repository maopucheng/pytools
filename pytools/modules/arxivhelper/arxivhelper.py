...
class ArxivHelper:
    def __init__(
        self,
        keywords_list=['continual learning'],
        history_filename='cache.pkl',
    ...
    def search(self, keyword):
        ...
        for item in soup.find('ol').find_all('li', attrs={'class': 'arxiv-result'}):
            try:
                title = item.find('p', attrs={'class': 'title'}).text.strip()
            except:
                title = ''
            try:
                authors = self.cleantext(
                    item.find('p', attrs={'class': 'authors'}).text.strip()
                ).replace('Authors:', '')
            except:
                authors = ''
            try:
                abstract = self.cleantext(
                    item.find('p', attrs={'class': 'abstract'}).text.strip()
                ).replace('Abstract:', '')
            except:
                abstract = ''
            try:
                comments = self.cleantext(
                    item.find('p', attrs={'class': 'comments'}).text.strip()
                ).replace('Comments:', '')
            except:
                comments = ''
            try:
                url = (
                    item.find('p', attrs={'class': 'list-title'})
                    .find('a')
                    .attrs['href']
                )
            except:
