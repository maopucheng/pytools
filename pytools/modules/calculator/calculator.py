...
class Calculator:
    ...
    def __init__(self, title='简易计算器 —— Car的皮皮', root_size=(320, 420), **kwargs):
        # 基本界面
    ...
    def pressnumber(self, number):
        ...
        if self.current_show.get() == '0':
            ...
            if len(self.current_show.get()) < self.max_show_len:
    ...
    def pressdp(self):
        ...
        if len(self.current_show.get().split('.')) == 1:
            if len(self.current_show.get()) < self.max_show_len:
                self.current_show.set(self.current_show.get() + '.')
    ...
    def modify(self, result):
        ...
        if len(result) > self.max_show_len:
            if len(result.split('.')[0]) > self.max_show_len:
                ...
                result = result[: self.max_show_len]
        return result
    ...
    def pressoperator(self, operator):
        # 取反
        if operator == '+/-':
            if self.current_show.get().startswith('-'):
                self.current_show.set(self.current_show.get()[1:])
            else:
                self.current_show.set('-' + self.current_show.get())
        # 取倒数
            ...
            try:
                result = 1 / float(self.current_show.get())
            except:
        ...
        # 求平方根
            ...
            try:
                result = math.sqrt(float(self.current_show.get()))
            except:
                result = 'illegal operation'
            result = self.modify(result)
            self.current_show.set(result)
    ...
    def run(self):
        ...
        label = tkinter.Label(
            self.root,
            textvariable=self.current_show,
            bg='black',
            anchor='e',
            bd=5,
            fg='white',
        ...
        button1_1 = tkinter.Button(
            text='MC', bg='#666', bd=2, command=lambda: self.pressoperator('MC')
        )
        button1_1.place(x=20, y=110, width=50, height=35)
        # ----Memory read
        button1_2 = tkinter.Button(
            text='MR', bg='#666', bd=2, command=lambda: self.pressoperator('MR')
        )
        button1_2.place(x=77.5, y=110, width=50, height=35)
        # ----Memory save
        button1_3 = tkinter.Button(
            text='MS', bg='#666', bd=2, command=lambda: self.pressoperator('MS')
        )
        button1_3.place(x=135, y=110, width=50, height=35)
        # ----Memory +
        button1_4 = tkinter.Button(
            text='M+', bg='#666', bd=2, command=lambda: self.pressoperator('M+')
        )
        ...
        button4_1 = tkinter.Button(
            text='4', bg='#bbbbbb', bd=2, command=lambda: self.pressnumber('4')
        )
        button4_1.place(x=20, y=245, width=50, height=35)
        # ----5
        button4_2 = tkinter.Button(
            text='5', bg='#bbbbbb', bd=2, command=lambda: self.pressnumber('5')
        )
        button4_2.place(x=77.5, y=245, width=50, height=35)
        # ----6
        button4_3 = tkinter.Button(
            text='6', bg='#bbbbbb', bd=2, command=lambda: self.pressnumber('6')
        )
        ...
        # ----取导数
        button4_5 = tkinter.Button(
            text='1/x', bg='#708069', bd=2, command=lambda: self.pressoperator('1/x')
        )
        button4_5.place(x=250, y=245, width=50, height=35)
        # --第五行
        # ----3
        button5_1 = tkinter.Button(
            text='3', bg='#bbbbbb', bd=2, command=lambda: self.pressnumber('3')
        )
        button5_1.place(x=20, y=290, width=50, height=35)
        # ----2
        button5_2 = tkinter.Button(
            text='2', bg='#bbbbbb', bd=2, command=lambda: self.pressnumber('2')
        )
        button5_2.place(x=77.5, y=290, width=50, height=35)
        # ----1
        button5_3 = tkinter.Button(
            text='1', bg='#bbbbbb', bd=2, command=lambda: self.pressnumber('1')
        )
        button5_3.place(x=135, y=290, width=50, height=35)
        # ----减
        button5_4 = tkinter.Button(
            text='-', bg='#708069', bd=2, command=lambda: self.pressoperator('-')
        )
        button5_4.place(x=192.5, y=290, width=50, height=35)
        # ----等于
        button5_5 = tkinter.Button(
            text='=', bg='#708069', bd=2, command=lambda: self.pressoperator('=')
        )
        button5_5.place(x=250, y=290, width=50, height=80)
        # --第六行
        # ----0
        button6_1 = tkinter.Button(
            text='0', bg='#bbbbbb', bd=2, command=lambda: self.pressnumber('0')
        )
        button6_1.place(x=20, y=335, width=107.5, height=35)
        # ----小数点
        button6_2 = tkinter.Button(
            text='.', bg='#bbbbbb', bd=2, command=lambda: self.pressdp()
        )
        button6_2.place(x=135, y=335, width=50, height=35)
        # ----加
        button6_3 = tkinter.Button(
            text='+', bg='#708069', bd=2, command=lambda: self.pressoperator('+')
        )
